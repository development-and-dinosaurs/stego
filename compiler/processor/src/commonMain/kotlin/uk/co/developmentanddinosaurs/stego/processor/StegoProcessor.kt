...
class StegoProcessor(
    ...
    private fun writeBaseComponentMetadata(symbols: Set<KSClassDeclaration>) {
        ...
        logger.info("Wrote base component metadata for: ${baseComponents.map { it.qualifiedName }.toList()}")
    }
    ...
    private fun toComponentMetadata(classDeclaration: KSClassDeclaration): ComponentMetadata {
        val stegoAnnotation = classDeclaration.annotations.first { it.shortName.asString().contains("Stego") }
        val stegoType = stegoAnnotation.arguments.first { it.name?.asString() == "type" }.value as String
        val superType =
