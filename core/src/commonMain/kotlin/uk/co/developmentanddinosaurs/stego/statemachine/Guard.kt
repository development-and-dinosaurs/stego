 ...
 */
data class EqualsGuard(val left: ValueReference, val right: ValueReference) : Guard {
 ...
 */
data class NotEqualsGuard(val left: ValueReference, val right: ValueReference) : Guard {
    override fun evaluate(context: Context, event: Event): Boolean {
        return left.resolve(context, event) != right.resolve(context, event)
    }
}
 ...
 */
data class GreaterThanGuard(val left: ValueReference, val right: ValueReference) : Guard {
    override fun evaluate(context: Context, event: Event): Boolean {
 ...
 */
data class LessThanGuard(val left: ValueReference, val right: ValueReference) : Guard {
    override fun evaluate(context: Context, event: Event): Boolean {
        val leftValue = left.resolve(context, event)
        val rightValue = right.resolve(context, event)
        if (leftValue !is Primitive || rightValue !is Primitive) return false
 ...
 */
data class GreaterThanOrEqualsGuard(val left: ValueReference, val right: ValueReference) : Guard {
    override fun evaluate(context: Context, event: Event): Boolean {
        val leftValue = left.resolve(context, event)
        val rightValue = right.resolve(context, event)
 ...
 */
data class LessThanOrEqualsGuard(val left: ValueReference, val right: ValueReference) : Guard {
    override fun evaluate(context: Context, event: Event): Boolean {
        val leftValue = left.resolve(context, event)
        val rightValue = right.resolve(context, event)
        if (leftValue == rightValue) return true
