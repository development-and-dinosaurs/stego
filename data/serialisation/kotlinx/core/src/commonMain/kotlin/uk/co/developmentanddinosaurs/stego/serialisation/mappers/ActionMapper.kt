 ...
 *
 * This class acts as a bridge between the data transfer layer and the core state machine logic. It
 * is configured with a registry that maps DTO types to their specific [ActionDtoMapper].
 *
 * To use, provide a map of any custom mappers. These will be merged with the [defaultMappers].
 * Providing a custom mapper for a default type will override it.
 *
 ...
 *   ```
 *
 * @param customMappers A map of custom mappers to be added to the default set. If a key in this map
 *   conflicts with a default mapper, the custom mapper will be used.
 */
class ActionMapper(
   ...
   * @throws StateMachineException if the DTO type is not a built-in type and is not found in the
   *   provided `mappers` registry.
   */
