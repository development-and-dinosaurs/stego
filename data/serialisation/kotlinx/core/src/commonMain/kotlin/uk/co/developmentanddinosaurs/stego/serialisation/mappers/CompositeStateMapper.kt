...
/**
 * The primary entry point for mapping a complete [StateMachineDefinitionDto] into its domain
 * [StateMachineDefinition] counterpart.
 *
 * This class acts as a composite mapper, holding a registry of specific [StateDtoMapper]
 * implementations. It uses a factory pattern for creating these mappers, allowing them to be aware
 * of the composite mapper itself. This is crucial for recursively mapping hierarchical state
 * machines, where a child state's mapper needs to delegate back to the main composite mapper to map
 * its own children.
 */
class CompositeStateMapper(
   ...
   *
   * This function handles the mapping of the initial state, the initial context, and then delegates
   * the mapping of each individual state DTO to the appropriate registered mapper.
   *
   * @param dto The complete state machine definition DTO to map.
   * @return The resulting domain [StateMachineDefinition].
   * @throws StateMachineException if any part of the mapping fails, such as an unmappable initial
   *   context value or an unregistered state DTO type.
   */
  ...
  /**
   * Maps a single [StateDto] to its domain [State] counterpart by delegating to the appropriate
   * mapper from the registry.
   *
