...
private object TestInvokable : Invokable {
  override suspend fun invoke(input: Map<String, Any?>) =
      throw NotImplementedError("Not used in this test")
}

class LogicStateMapperTest :
    BehaviorSpec({
      ...
      val actionMapper =
          ActionMapper(
              customMappers =
                  mapOf(
                      TestActionDto::class to
                          ActionDtoMapper { dto -> TestAction((dto as TestActionDto).data) },
                  ),
          )
      ...
      Given("a LogicStateMapper") {
        val mapper =
            LogicStateMapper(actionMapper, invokableMapper, transitionMapper, compositeStateMapper)
