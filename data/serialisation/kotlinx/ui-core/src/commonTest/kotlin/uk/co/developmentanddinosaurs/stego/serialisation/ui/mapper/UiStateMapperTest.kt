...
class UiStateMapperTest : BehaviorSpec({
    Given("a UiStateMapper") {
        ...
        val invokableMapper = InvokableDefinitionMapper(mapOf("invoke-name" to Invokable { InvokableResult.Success() }))
        val transitionMapper = TransitionMapper(actionMapper)
        val uiNodeMapper = CompositeUiNodeMapper(
            simpleMappers = mapOf<KClass<out UiNodeDto>, UiNodeMapper>(
        ...
        and("a complete UiStateDto") {
            val dto = UiStateDto(
                ...
                invoke = InvokableDefinitionDto("invoke-id", "invoke-name"),
                initial = "start",
