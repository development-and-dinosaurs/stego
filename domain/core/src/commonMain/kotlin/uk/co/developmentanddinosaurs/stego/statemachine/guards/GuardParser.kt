 ...
 * 1. **Strict Parenthesizing**: All expressions, simple or composite, must be enclosed in
 *    parentheses, with the only exception being the unary `NOT` (`!`) operator at the start of an
 *    expression.
 ...
 * 3. **Operands**: Can be literals (e.g., `5`, `"hello"`, `true`) or dynamic values resolved from
 *    context (`{context.key}`) or events (`{event.key}`).
 */
internal object GuardParser {
  /**
   * Operator map for simple comparisons. The order is critical to ensure longest-match-first (e.g.,
   * '>=' before '>').
   */
   ...
   * @throws IllegalArgumentException if the expression has invalid syntax or unbalanced
   *   parentheses.
   */
  fun parse(expression: String): Guard {
    ...
    findSplitPoint(innerExpression, "||")?.let {
      return OrGuard(
          parse(innerExpression.substring(0, it)),
          parse(innerExpression.substring(it + 2).trim()),
      )
    }
    findSplitPoint(innerExpression, "&&")?.let {
      return AndGuard(
  ...
  /**
   * Checks if an expression is fully and exclusively enclosed by a single pair of matching
   * parentheses. For example, `(a && b)` is true, but `(a) && (b)` is false.
   */
