  ...
  init {
    ...
    Given("a deeply nested hierarchical state machine") {
      ...
      When("the engine is created") {
        ...
        Then("it should enter the deepest initial state") { engine.output.value.state shouldBe l5 }
    ...
    Given("a state with an internal self-transition") {
      ...
      val state =
          LogicState(
              ...
              onExit = listOf(exitAction),
              on =
                  mapOf("INTERNAL_EVENT" to listOf(Transition(actions = listOf(transitionAction)))),
          )
      val definition =
          StateMachineDefinition(
              initial = "Internal",
