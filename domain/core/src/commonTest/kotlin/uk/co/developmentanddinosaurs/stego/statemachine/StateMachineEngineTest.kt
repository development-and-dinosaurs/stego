...
/**
 * An action that introduces an artificial delay. NOTE: This uses Thread.sleep(), which is generally
 * discouraged with coroutines. However, it's the most straightforward way to test event queuing
 * without a major refactoring to make the Action interface suspendable.
 */
...
class StateMachineEngineTest :
    BehaviorSpec({
      ...
      Given("a state machine with a transition to a non-existent state") {
        val stateA =
            LogicState(id = "StateA", on = mapOf("EVENT" to listOf(Transition(target = "StateC"))))
        val definition =
            StateMachineDefinition(
                initial = "StateA",
        ...
        When("the engine is created") {
          ...
          Then("a state machine exception should be thrown") {
            exception.message shouldBe
                "State 'StateA' has a transition to non-existent target state 'StateC'."
          }
        }
      }
      ...
      Given("a state machine with a nested non-existent initial state") {
        val parentState =
            LogicState(
                id = "Parent",
                initial = "Child",
                states = mapOf("NotChild" to LogicState(id = "NotChild")),
            )
        val definition =
            StateMachineDefinition(
                initial = "Parent",
      ...
      Given("a state machine with an initial and a next state") {
        val initialState =
            LogicState(id = "Initial", on = mapOf("NEXT" to listOf(Transition(target = "Next"))))
      ...
      Given("a state machine with a guarded transition") {
        ...
        val definition =
            StateMachineDefinition(
                initial = "Guarded",
                states =
                    mapOf("Guarded" to guardedState, "Next" to nextState, "Other" to otherState),
            )
      ...
      Given("a state machine with a transition that has an action") {
        ...
        val initialState =
            LogicState(id = "Initial", on = mapOf("ACTION_EVENT" to listOf(transition)))
        val nextState = LogicState(id = "Next")
        val definition =
            StateMachineDefinition(
                initial = "Initial",
      ...
      Given("a state machine with a transition that has multiple actions") {
        ...
        val initialState =
            LogicState(id = "Initial", on = mapOf("MULTI_ACTION_EVENT" to listOf(transition)))
        val nextState = LogicState(id = "Next")
        val definition =
            StateMachineDefinition(
                initial = "Initial",
        ...
        val initialState =
            LogicState(
                ...
                on =
                    mapOf(
                        "MOVE" to
                            listOf(Transition(target = "Next", actions = listOf(transitionAction)))
                    ),
            )
      ...
      Given("a state with an invokable service") {
        ...
        When("the engine is created") {
          ...
          Then(
              "the invokable should be executed and the resulting event should cause a transition"
          ) {
      ...
      Given("a state with a cancellable invokable service") {
        ...
        val idleState =
            LogicState(
                id = "Idle",
                on = mapOf("done.invoke.testInvoke" to listOf(Transition("FailedTestState"))),
            )
        val loadingState =
            LogicState(
                id = "Loading",
      ...
      Given("a hierarchical state machine with a transition on the parent") {
        ...
        When("the engine is in the child state and an event is sent") {
          ...
          Then(
              "the engine should find the transition on the parent and move to the correct state"
          ) {
      ...
      Given("a hierarchical transition between sibling states") {
        ...
        When("a transition occurs between the siblings") {
          ...
          Then("only the relevant entry and exit actions should be executed") {
            engine.output.first { it.state == childTwo }.context.get("trace") shouldBe
                listOf(
                    "parent_entry",
                    "childOne_entry",
                    "childOne_exit",
      ...
      Given("a deeply nested hierarchical state machine") {
        ...
        When("the engine is created") {
          ...
          Then("it should execute all entry actions in order") {
            engine.output.value.context.get("trace") shouldBe
                listOf(
                    "l1_entry",
                    "l2_entry",
                    "l3_entry",
        ...
        val stateA =
            LogicState(
                ...
                on =
                    mapOf(
                        "EVENT_A" to
                            listOf(Transition(target = "StateB", actions = listOf(sendAction))),
                    ),
            )
        val definition =
            StateMachineDefinition(
      ...
      Given("a hierarchical transition from a child to its parent") {
        ...
        When("a transition occurs from the child to the parent") {
          ...
          Then("only the child's exit action should be called") {
            val finalOutput = engine.output.value
            finalOutput.context.get("trace") shouldBe
                listOf("parent_entry", "child_entry", "child_exit")
      ...
      Given("a state with an internal self-transition") {
        ...
        val state =
            LogicState(
                ...
                on =
                    mapOf(
                        "INTERNAL_EVENT" to listOf(Transition(actions = listOf(transitionAction)))
                    ),
            )
        val definition =
            StateMachineDefinition(
