...
/**
 * A functional interface that defines a contract for mapping a specific [ActionDto]
 * to its corresponding domain [Action].
 *
 * This is the core component used in the [ActionMapper] registry to handle the conversion
 * of different action types.
 *
 * ### Implementation as a Class
 * For mappers with their own dependencies or complex logic, you can implement this
 * interface as a full class:
 * ```
 ...
 * Because this is a `fun interface`, for simple, self-contained mappers, you can
 * provide a concise lambda directly in the [ActionMapper] registry:
 * ```
 * val mappers = mapOf(
